...
- **回测模式**：`populate_indicators` 会对整个历史数据调用一次。
- **实盘模式**：每生成一个新K线（如每分钟、每小时）就会调用一次。

因此在实盘时，你可以在该函数中加入**实时判断与处理逻辑**，而在回测中需要用 `for row in dataframe.iterrows()` 遍历历史数据。
...
Freqtrade 会在满足该条件的时刻，用**市价单或限价单**发起交易。
...
`populate_exit_trend` 是你定义**何时平仓**（卖出）的函数，操作逻辑与买入类似，只是设置的是 `sell` 列：
