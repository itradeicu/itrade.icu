...
1. **没有明显高点/低点对应关系**
   → 比如价格小幅创新高，而 CCI 的顶部非常接近或略低，无法确认是否构成背离。

2. **背离周期过短、容易误判**
   → 小波段行情中，CCI 可能经常与价格错开波峰谷，造成“假背离”。

3. **震荡市中出现“伪背离”**
   → 多头或空头能量本就不强，背离信号并不代表有效反转。
...
* **结合 EMA（指数移动平均线）**
  例如：只有当 CCI 发出超卖信号（< -100）且价格位于 EMA 20 之上时，才考虑做多；反之，CCI 超买（> +100）且价格位于 EMA 20 之下时，考虑做空。
